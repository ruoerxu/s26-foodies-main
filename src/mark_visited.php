...
// Insert — ignore if already marked visited
$stmt = mysqli_prepare($conn,
    'INSERT IGNORE INTO visited_restaurants (party_id, restaurant_id) VALUES (?, ?)');
if (!$stmt) {
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'DB error']);
    exit;
...
if (!mysqli_stmt_execute($stmt)) {
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Failed to mark as visited']);
    mysqli_stmt_close($stmt);
    exit;
}
mysqli_stmt_close($stmt);
...
echo json_encode(['success' => true, 'message' => 'Restaurant marked as visited']);

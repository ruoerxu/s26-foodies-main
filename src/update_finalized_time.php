...
$stmt = mysqli_prepare($conn, 'UPDATE finalized_plan SET meeting_time = ?, updated_at = CURRENT_TIMESTAMP WHERE party_id = ?');
if (!$stmt) { http_response_code(500); echo json_encode(['success' => false, 'message' => 'DB error']); exit; }
mysqli_stmt_bind_param($stmt, 'si', $time, $partyId);
mysqli_stmt_execute($stmt);
mysqli_stmt_close($stmt);
mysqli_close($conn);

http_response_code(200);
echo json_encode(['success' => true, 'message' => 'Meeting time updated successfully']);

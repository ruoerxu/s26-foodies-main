...
$partyName = isset($_GET['party_name']) ? trim((string)$_GET['party_name']) : '';
if ($partyName === '') {
...
$isActive = false;
$selectedCuisines = [];
$stmt = mysqli_prepare($conn, 'SELECT is_active, selected_cuisines FROM group_sessions WHERE party_id = ? LIMIT 1');
if (!$stmt) {
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Database error']);
    exit;
...
mysqli_stmt_execute($stmt);
mysqli_stmt_bind_result($stmt, $activeValue, $selectedCuisinesJson);
if (mysqli_stmt_fetch($stmt)) {
    $isActive = ((int)$activeValue) === 1;
    if ($isActive && $selectedCuisinesJson !== null) {
        $decodedCuisines = json_decode($selectedCuisinesJson, true);
        if (is_array($decodedCuisines)) {
            $selectedCuisines = $decodedCuisines;
        } else {
            $isActive = false;
            $selectedCuisines = [];
        }
